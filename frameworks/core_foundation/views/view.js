...
/**
  ...
  the pane until they find an event which implements the event handler method (and does not
  return NO). (Again, to handle an event but allow it to continue bubbling, return NO.)
  ...
  Data-drag events
  ----
  Browsers implement a parallel system of events for drags that bring something with them: for
  ...
  set of "data drag" events. These behave much like mouse events.
  ...
  - `dataDragDropped` -- If the last hover event's dropEffect was set correctly, this event will
    give the view access to the data that was dropped.
  ...
  `acceptsKeyPane` set to YES.

  SproutCore implements a set of higher-level keyboard events for action keys such as *tab*,
  *enter,* and the arrow keys. These are not triggered automatically, but you can gain access
  to them by proxying the keyboard event of your choice to `interpretKeyEvent`. For example:
  ...
  - `moveLeftAndModifySelection` -- Shift + the left key
  - `selectAll` -- Ctrl + A / Cmd + A

  Et cetera. For a full list of available methods, see the key values on SC.BASE_KEY_BINDINGS
